...
dataDiag <- read_delim(file = here::here("compiled_datasets/loc_all_raw_pre-qc.txt"),
                       delim = "\t") |> 
  select(CAMPAIGN) |> 
  distinct() |> 
  mutate(db_diag = "y") |> 
...
dataDive <- read_delim(file = here::here("compiled_datasets/dive_all_raw_pre-qc.txt"),
                       delim = "\t") |> 
  select(CAMPAIGN) |> 
  distinct() |> 
  mutate(db_dive = "y") |> 
...
dMeta <- list(dMeta, dataDiag, dataDive, meta) |> # Cross-referencing decade metadata to see if we have corresponding location/dive data, and imos meta for each campaign
