...
dive <- fread(here("compiled_raw_datasets/ctd_all_raw_pre-qc.txt"))  %>% 
  mutate(id = ref,
  date = end_date
  ) 
tdive <- dive %>% 
...
if(file_exists("./processed_datasets/loc_ssm_ctd.csv")){
  camp_comp <- fread("./processed_datasets/loc_ssm_ctd.csv") %>% 
    pull(id) %>% 
    unique()
  dive <- dive %>% 
...
if(file_exists("./processed_datasets/fit_all_ctd.Rdata")){
  # fit_new <- fit_all
  load(file= "./processed_datasets/fit_all_ctd.Rdata")
  fit_all <- bind_rows(fit_all, fit_new)
}

##save the output
save(fit_all, file= "./processed_datasets/fit_all_ctd.Rdata")
...
ssm_ctd <- left_join(ssm, idmeta, by=c("id"))

# Append to existing processed dataset if available
if(file_exists("./processed_datasets/ssm_ctd.Rdata")){
  ssm_ctd_new <- ssm_6h
  d1_new <- d1
  load(file = "./processed_datasets/ssm_ctd.Rdata")
  ssm_ctd <- bind_rows(ssm_ctd, ssm_ctd_new)
  d1 <- bind_rows(d1, d1_new)
}

save(ssm_ctd, d1, file = "./processed_datasets/ssm_ctd.Rdata")
fwrite(ssm_ctd, file = "./processed_datasets/loc_ssm_ctd.csv", sep = ",")
